...
def parse_agrs():
    ...
    # Data input settings
    parser.add_argument('--image_dir', type=str, default='/media/sun/DATA/data_2T/physionet.org/files/mimic-cxr-jpg/2.1.0/', help='the path to the directory containing the data.')
    parser.add_argument('--ann_path', type=str, default='/media/sun/DATA/data_2T/physionet.org/files/mimic-cxr-jpg/2.1.0/updated_mimic_annotation.json', help='the path to the directory containing the data.')
    ...
    parser.add_argument('--beam_size', type=int, default=2, help='the beam size when beam searching.')
    ...
    parser.add_argument('--save_dir', type=str, default='results/mimic_cxr/base_cmn_rl2', help='the patch to save the models.')
...
ckpt = torch.load('results/mimic_cxr_0912/model_best.pth')
print(ckpt['epoch'])
...
path = '/media/sun/DATA/data_2T/physionet.org/files/mimic-cxr-jpg/2.1.0/p19/p19075045/s52690612/b078a488-d4e59bf9-bdd410ac-f6f0a126-8d0fca7e.jpg'
...
output = model(img.unsqueeze(0), mode='sample')
...
interpolate = transforms.Resize((arr.shape[0], arr.shape[1]))

viridis = cm.get_cmap('jet', 1000)
# viridis = mpl.colormaps.get_cmap('jet', 1000)
cmap = viridis
